...
- **Largest files is a retained-map projection, not a review scan:** it may
  inspect only file nodes from a completed internal compact tree, order by
  on-disk size with a raw-path tie break, and expose Quick Look + Reveal only.
  Never include external/Folder Lens maps, persist results, call Spotlight,
